...
export async function scrapeNuLiga({
  ...
  for (const groupLink of groupLinks) {
    ...
    if (applyChanges && supabaseClient) {
      ...
      // ✅ WICHTIG: Übergebe leagueUrl als source_url, damit jeder Spieltag weiß, von welcher URL er stammt
      await upsertMatchdays(supabaseClient, parsed.matches, teamIdRegistry, groupMeta, leagueUrl);
...
export function buildMatchPayload(match, teamIdMap, meta, sourceUrl = null) {
  ...
  return {
    ...
    source_url: sourceUrl || null, // ✅ NEU: URL, von der dieser Spieltag stammt
    source_type: 'nuliga', // ✅ NEU: Typ der Quelle (nuliga, tvm, manual, etc.)
    updated_at: new Date().toISOString()
  };
}

export async function upsertMatchdays(supabaseClient, matches, teamIdMap, meta, sourceUrl = null) {
  ...
  const payload = matches.map((match) => buildMatchPayload(match, teamIdMap, meta, sourceUrl));
