...
export async function scrapeNuLiga({
  ...
  for (const groupLink of groupLinks) {
    ...
    const { mapped: teamIdRegistry, unmapped } = mapTeamsToIds(parsed.teamsDetailed, teamDirectory);
...
export function mapTeamsToIds(teamRecords = [], teamDirectory = { byKey: new Map(), list: [] }) {
    ...
    if (teamDirectory.byKey && teamDirectory.byKey.size > 0) {
      ...
      for (const key of candidateKeys) {
        if (key && teamDirectory.byKey.has(key)) {
          matchRecord = teamDirectory.byKey.get(key)[0];
          break;
        }
      }
      ...
      if (!matchRecord && teamDirectory.list.length > 0) {
        ...
        if (bestMatch && bestMatch.id && bestScore >= 0.9) {
