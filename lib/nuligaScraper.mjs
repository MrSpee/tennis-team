...
export async function scrapeNuLiga({
  ...
  for (const groupLink of groupLinks) {
    ...
    if (applyChanges && supabaseClient) {
      ...
      // ✅ WICHTIG: Übergebe leagueUrl an upsertTeamSeasons, damit die URL auf Gruppenebene gespeichert wird
      await upsertTeamSeasons(supabaseClient, Array.from(teamIdRegistry.values()), groupMeta, leagueUrl);
...
export async function fetchGroupLinks({ leagueUrl, groupFilter, fetchImpl, log = () => {} }) {
  ...
  $(anchorSelector).each((_, anchor) => {
      ...
      if ($row.length) {
        ...
        if ($cells.length > 0) {
          ...
          if (firstCell[0] !== $groupCell[0]) {
            ...
            } else if (/^(Herren|Damen)\b(?!\s+\d+)/.test(firstCellText)) {
              // "Herren" oder "Damen" ohne Zahl (z.B. "Herren Mittelrheinliga")
              const match = firstCellText.match(/^(Herren|Damen)\b/i);
              if (match) {
                category = match[1]; // Nur "Herren" oder "Damen"
              }
            }
          } else {
            ...
            if ($prev.length) {
              ...
              if (/^(Herren|Damen)\s+\d+/.test(prevText)) {
                category = prevText;
              } else if (/^(Herren|Damen)\b(?!\s+\d+)/.test(prevText)) {
                // "Herren" oder "Damen" ohne Zahl
                const match = prevText.match(/^(Herren|Damen)\b/i);
                if (match) {
                  category = match[1];
                }
              }
    ...
    if (!category && $groupCell.length) {
      ...
      if ($prevRow && $prevRow.length) {
        ...
        } else if (/^(Herren|Damen)\b(?!\s+\d+)/.test(prevRowText)) {
          // "Herren" oder "Damen" ohne Zahl
          const match = prevRowText.match(/^(Herren|Damen)\b/i);
          if (match) {
            category = match[1];
          }
        }
    ...
    if (!category && $groupCell.length) {
      ...
      if ($nextRow && $nextRow.length) {
        ...
        } else if (/^(Herren|Damen)\b(?!\s+\d+)/.test(nextRowText)) {
          // "Herren" oder "Damen" ohne Zahl
          const match = nextRowText.match(/^(Herren|Damen)\b/i);
          if (match) {
            category = match[1];
          }
        }
  ...
  if (groupLinks.length === 0) {
    // VERBESSERT: Detailliertere Fehlermeldung
    const error = new Error('Keine Gruppenlinks auf der Ligaübersicht gefunden. Prüfe die URL oder den Gruppenfilter.');
    error.details = {
      leagueUrl,
      groupFilter,
      anchorCount: $(anchorSelector).length,
      pageTitle: $('title').text() || 'Unbekannt',
      pageHasContent: html.length > 0
    };
    log(`❌ Fehler: ${error.message}`, error.details);
    throw error;
  }

  log(`✅ Gefundene Gruppen-Links (${groupLinks.length}): ${groupLinks.map((link) => link.label).join(', ')}`);
  return groupLinks;
}
  ...
  for (const { type, element } of tables) {
    ...
    if (type === 'matches') {
      ...
      rows.forEach(({ cells, element: rowElement }, index) => {
        ...
        // ✅ FIX: nuLiga-Tabelle hat 11 Spalten (Index 0-10)
        // Spalten: Wochentag, Datum+Zeit, Nr., Spielort, Platz, Heimmannschaft, Gastmannschaft, Matchpunkte, Sätze, Spiele, Spielbericht
        if ($cols.length < 11) {
          skippedRows++;
        ...
        // ✅ FIX: Spalten-Indizierung korrigiert (nuLiga hat 11 Spalten, Index 0-10)
        // Index 0: Wochentag (Sa., So.) oder leer
        // Index 1: Datum+Zeit (22.11.2025 18:00)
        // Index 2: Nr. (537)
        // Index 3: Spielort
        // Index 4: Platz (1+2)
        // Index 5: Heimmannschaft
        // Index 6: Gastmannschaft
        // Index 7: Matchpunkte (2:4)
        // Index 8: Sätze (7:9)
        // Index 9: Spiele (60:63)
        // Index 10: Spielbericht (letzte Spalte)
        const dateCellText = normaliseText($cols.eq(1).text());
        const matchNumberRaw = normaliseText($cols.eq(2).text());
        const venueRaw = normaliseText($cols.eq(3).text());
        const courtRaw = normaliseText($cols.eq(4).text());

        const homeMeta = extractTeamMetaFromCell($cols.eq(5));
        const awayMeta = extractTeamMetaFromCell($cols.eq(6));
        ...
        const matchPoints = parseScore(normaliseText($cols.eq(7).text()));
        const setScore = parseScore(normaliseText($cols.eq(8).text()));
        const gameScore = parseScore(normaliseText($cols.eq(9).text()));
...
export function deriveGroupMeta(groupPageHtml, groupLink, seasonLabel = DEFAULT_SEASON_LABEL) {
  ...
  if (!category) {
    // Pattern 4: "Herren" oder "Damen" gefolgt von Text (z.B. "Herren Mittelrheinliga")
    // WICHTIG: Nur wenn es NICHT "Herren/Damen" + Zahl ist (das wurde schon geprüft)
    const categoryMatch4 = headerText.match(/\b(Herren|Damen)\s+(?!\d+)([A-Za-z]+)/i);
    if (categoryMatch4) {
      // Spezielle Ligen ohne Altersklasse: "Mittelrheinliga", "Verbandsliga", etc.
      const gender = categoryMatch4[1];
      const leagueType = categoryMatch4[2];
      
      // Wenn es eine Liga ohne Altersklasse ist, verwende nur "Herren" oder "Damen"
      if (/Mittelrheinliga|Verbandsliga|Oberliga|Bundesliga/i.test(leagueType)) {
        category = gender; // "Herren" oder "Damen" ohne Zahl
      }
    }
  }
  
  if (!category) {
    // Pattern 5: "Herren" oder "Damen" allein (ohne Zahl, ohne Liga-Typ)
    const categoryMatch5 = headerText.match(/\b(Herren|Damen)\b(?!\s+\d+)(?!\s+[A-Za-z]+)/i);
    if (categoryMatch5) {
      category = categoryMatch5[1]; // "Herren" oder "Damen" ohne Zahl
    }
  }
  ...
  // Liga-Extraktion: Alles zwischen Kategorie und "Gr."
  let league = 'Unbekannte Liga';
  
  // Pattern 1: "Herren 30 [LIGA] Gr." oder "Damen 40 [LIGA] Gr."
  const leagueMatch1 = headerText.match(/Herren\s+\d+\s+(.*?)\s+Gr\./i) || 
                       headerText.match(/Damen\s+\d+\s+(.*?)\s+Gr\./i);
  if (leagueMatch1) {
    league = leagueMatch1[1].trim();
  } else {
    // Pattern 2: Fallback - Entferne Saison und Kategorie, dann extrahiere Liga vor "Gr."
    let leagueText = headerText;
    
    // Entferne Saison-Pattern (z.B. "Winter 2025/2026")
    leagueText = leagueText.replace(/\b(Winter|Sommer|Herbst|Frühjahr|Fruehjahr)\s+\d{4}(?:\/\d{2,4})?\s*/i, '');
    
    // Entferne Kategorie-Pattern (wenn bereits erkannt)
    if (category) {
      // Entferne Kategorie am Anfang oder mit Leerzeichen davor
      const categoryPattern = new RegExp(`\\b${category.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+`, 'i');
      leagueText = leagueText.replace(categoryPattern, '');
    }
    
    // Entferne "TVM" am Anfang (falls vorhanden)
    leagueText = leagueText.replace(/^TVM\s+/i, '');
    
    // Extrahiere Liga vor "Gr."
    const leagueMatch2 = leagueText.match(/(.*?)\s+Gr\./i);
    if (leagueMatch2) {
      league = leagueMatch2[1].trim();
    } else {
      // Letzter Fallback: Alles vor "Gr." (auch wenn es nicht perfekt ist)
      const leagueMatch3 = headerText.match(/(.*?)\s+Gr\.\s*\d+/i);
      if (leagueMatch3) {
        league = leagueMatch3[1].trim();
      }
    }
  }
...
export async function upsertTeamSeasons(supabaseClient, teamIds, meta, sourceUrl = null) {
...
export function buildMatchPayload(match, teamIdMap, meta) {
  ...
  return {
    ...
    meeting_id: match.meetingId || match.meeting_id || null, // ✅ WICHTIG: meeting_id muss gespeichert werden!
...
export async function upsertMatchdays(supabaseClient, matches, teamIdMap, meta) {
  ...
  // ✅ ENTFERNT: sourceUrl Parameter, da URL jetzt in team_seasons gespeichert wird
  const payload = matches.map((match) => buildMatchPayload(match, teamIdMap, meta));

  // ✅ WICHTIG: upsert mit onConflict 'id' aktualisiert auch meeting_id, wenn vorhanden
  // Das bedeutet: Wenn ein Match bereits existiert, wird die meeting_id aktualisiert, wenn sie im Payload vorhanden ist
  const { error } = await supabaseClient.from('matchdays').upsert(payload, { 
    onConflict: 'id',
    // ✅ Ignoriere keine Spalten - alle Felder werden aktualisiert, inkl. meeting_id
  });
...
function extractLkValue(...sources) {
  for (const source of sources) {
    ...
    // WICHTIG: Sehr restriktiv - nur wenn explizit LK-Kontext vorhanden
    match = text.match(/\b(\d{1,2})\b/);
    if (match && parseInt(match[1]) >= 1 && parseInt(match[1]) <= 25) {
      const num = parseInt(match[1]);
      const context = text.toLowerCase();
      
      // KRITISCH: Nur akzeptieren wenn:
      // 1. Explizit "LK" oder "Leistungsklasse" im Text steht
      // 2. NICHT "Position" oder "Pos" (das wäre die Position, nicht die LK!)
      // 3. Zahl zwischen 7-25 (LK < 7 ist sehr selten und verdächtig)
      if ((context.includes('lk') || context.includes('leistungsklasse')) &&
          !context.includes('position') && !context.includes('pos')) {
        // Warnung für sehr niedrige LK-Werte
        if (num < 7) {
          console.warn(`⚠️ Verdächtig niedrige LK extrahiert: ${num} aus "${text}"`);
        }
        return `${num}.0`;
      }
    }
...
function extractPlayersFromCell($, cell, baseUrl = 'https://tvm.liga.nu') {
  ...
  if (foundLinks.length > 0) {
    foundLinks.forEach(({ $anchor, rawLabel }) => {
      ...
      let lk = extractLkValue(meta, rawLabel);
      
      // 🎾 LK-VALIDIERUNG: Warnung bei verdächtigen Werten
      if (lk) {
        const lkValue = parseFloat(lk);
        if (lkValue < 7) {
          console.warn(`⚠️ Verdächtig niedrige LK extrahiert für ${name}: ${lk} (Position: ${position})`);
          console.warn(`   Kontext: "${rawLabel}" | Meta: "${meta}"`);
          console.warn(`   ⚠️ Möglicherweise wurde die Position (${position}) statt der LK extrahiert!`);
        } else if (lkValue > 20) {
          console.warn(`⚠️ Verdächtig hohe LK extrahiert für ${name}: ${lk}`);
          console.warn(`   Kontext: "${rawLabel}" | Meta: "${meta}"`);
        }
      }
    ...
    if (playerMatches) {
      playerMatches.forEach((playerText) => {
        ...
        const meta = nameMatch && nameMatch[2] ? nameMatch[2].trim() : null;
        const position = extractPositionFromMeta(meta);
        let lk = extractLkValue(meta, trimmed);
        ...
        if (lk) {
          ...
          if (lkValue < 7) {
            console.warn(`⚠️ Verdächtig niedrige LK extrahiert für ${name}: ${lk} (Position: ${position})`);
            console.warn(`   Kontext: "${trimmed}" | Meta: "${meta}"`);
            console.warn(`   ⚠️ Möglicherweise wurde die Position (${position}) statt der LK extrahiert!`);
          } else if (lkValue > 20) {
            console.warn(`⚠️ Verdächtig hohe LK extrahiert für ${name}: ${lk}`);
            console.warn(`   Kontext: "${trimmed}" | Meta: "${meta}"`);
      ...
      let lk = extractLkValue(cellText);
      
      // 🎾 LK-VALIDIERUNG: Warnung bei verdächtigen Werten
      if (lk) {
        const lkValue = parseFloat(lk);
        if (lkValue < 7) {
          console.warn(`⚠️ Verdächtig niedrige LK extrahiert für ${name}: ${lk}`);
          console.warn(`   Kontext: "${cellText}"`);
          console.warn(`   ⚠️ Möglicherweise wurde die Position statt der LK extrahiert!`);
        } else if (lkValue > 20) {
          console.warn(`⚠️ Verdächtig hohe LK extrahiert für ${name}: ${lk}`);
          console.warn(`   Kontext: "${cellText}"`);
        }
...
export function parseMeetingReport(html) {
  ...
  $table.find('tr').each((_, row) => {
    ...
    if (currentSection === 'singles' && cells.length === 10) {
      ...
      // ✅ VERBESSERT: Bei Walkover-Matches versuche auch, Spieler aus der gesamten Zeile zu extrahieren
      // (manchmal sind Spieler in anderen Zellen oder als versteckte Links vorhanden)
      let finalHomePlayers = homePlayers;
      let finalAwayPlayers = awayPlayers;
      
      // Wenn keine Spieler gefunden wurden, versuche die gesamte Zeile zu durchsuchen
      if (homePlayers.length === 0 && !isPlayerNotPlayed(homeCellText)) {
        // Versuche alle Links in der gesamten Zeile zu finden
        const $row = $(cells[1]).closest('tr');
        const allLinksInRow = $row.find('a');
        if (allLinksInRow.length > 0) {
          console.log(`[nuligaScraper] 🔍 Versuche Spieler aus gesamter Zeile zu extrahieren (Home)`);
          // Versuche Spieler aus anderen Zellen der Zeile zu extrahieren
          $row.find('td').each((_, cell) => {
            const cellPlayers = extractPlayersFromCell($, cell, baseUrl);
            if (cellPlayers.length > 0 && finalHomePlayers.length === 0) {
              finalHomePlayers = cellPlayers;
              console.log(`[nuligaScraper] ✅ Home-Spieler aus anderer Zelle extrahiert:`, cellPlayers.map(p => p.name));
            }
          });
        }
      }
      
      if (awayPlayers.length === 0 && !isPlayerNotPlayed(awayCellText)) {
        // Versuche alle Links in der gesamten Zeile zu finden
        const $row = $(cells[3]).closest('tr');
        const allLinksInRow = $row.find('a');
        if (allLinksInRow.length > 0) {
          console.log(`[nuligaScraper] 🔍 Versuche Spieler aus gesamter Zeile zu extrahieren (Away)`);
          // Versuche Spieler aus anderen Zellen der Zeile zu extrahieren
          $row.find('td').each((_, cell) => {
            const cellPlayers = extractPlayersFromCell($, cell, baseUrl);
            if (cellPlayers.length > 0 && finalAwayPlayers.length === 0) {
              finalAwayPlayers = cellPlayers;
              console.log(`[nuligaScraper] ✅ Away-Spieler aus anderer Zelle extrahiert:`, cellPlayers.map(p => p.name));
            }
          });
        }
      }
      
      const homeForfeitFinal = finalHomePlayers.length === 0 && isPlayerNotPlayed(homeCellText);
      const awayForfeitFinal = finalAwayPlayers.length === 0 && isPlayerNotPlayed(awayCellText);
      let walkover = null;
      if (awayForfeitFinal && !homeForfeitFinal) {
        walkover = { winner: 'home', reason: awayCellText || 'Gast nicht angetreten' };
      } else if (homeForfeitFinal && !awayForfeitFinal) {
        walkover = { winner: 'guest', reason: homeCellText || 'Heim nicht angetreten' };
      }
      
      // DEBUG: Log extrahierte Spieler
      if (finalHomePlayers.length === 0 || finalAwayPlayers.length === 0) {
        console.log(`[nuligaScraper] ⚠️ Keine Spieler extrahiert für Einzel Match #${numbers.home}:`, {
          ...
          homePlayers: finalHomePlayers.length,
          awayPlayers: finalAwayPlayers.length,
          walkover: walkover ? `Ja (Winner: ${walkover.winner})` : 'Nein'
        });
      } else {
        console.log(`[nuligaScraper] ✅ Spieler extrahiert für Einzel Match #${numbers.home}:`, {
          home: finalHomePlayers.map(p => ({ name: p.name, lk: p.lk, position: p.position })),
          away: finalAwayPlayers.map(p => ({ name: p.name, lk: p.lk, position: p.position })),
          walkover: walkover ? `Ja (Winner: ${walkover.winner})` : 'Nein'
        });
      }
      ...
      finalHomePlayers.forEach(player => {
      ...
      finalAwayPlayers.forEach(player => {
        if (player.name && player.lk) {
          const normalized = normalizeName(player.name);
          if (normalized) {
            playerLkLookup.set(normalized, player.lk);
    ...
    if (currentSection === 'doubles' && cells.length === 12) {
      ...
      // Wenn keine Spieler gefunden wurden, versuche die gesamte Zeile zu durchsuchen
      if (homePlayers.length === 0 && !isPlayerNotPlayed(homeCellText)) {
        const $row = $(cells[2]).closest('tr');
        $row.find('td').each((_, cell) => {
          const cellPlayers = extractPlayersFromCell($, cell, baseUrl);
          if (cellPlayers.length > 0 && finalHomePlayers.length === 0) {
            finalHomePlayers = cellPlayers;
            console.log(`[nuligaScraper] ✅ Home-Spieler aus anderer Zelle extrahiert (Doppel):`, cellPlayers.map(p => p.name));
          }
        });
      }
      
      if (awayPlayers.length === 0 && !isPlayerNotPlayed(awayCellText)) {
        const $row = $(cells[5]).closest('tr');
        $row.find('td').each((_, cell) => {
          const cellPlayers = extractPlayersFromCell($, cell, baseUrl);
          if (cellPlayers.length > 0 && finalAwayPlayers.length === 0) {
            finalAwayPlayers = cellPlayers;
            console.log(`[nuligaScraper] ✅ Away-Spieler aus anderer Zelle extrahiert (Doppel):`, cellPlayers.map(p => p.name));
          }
        });
      }
      
      const homeForfeit = finalHomePlayers.length === 0 && isPlayerNotPlayed(homeCellText);
      const awayForfeit = finalAwayPlayers.length === 0 && isPlayerNotPlayed(awayCellText);
      let walkover = null;
      ...
      finalHomePlayers = finalHomePlayers.map(player => {
      ...
      finalAwayPlayers = finalAwayPlayers.map(player => {
        if (!player.lk && player.name) {
          const normalized = normalizeName(player.name);
          const lk = playerLkLookup.get(normalized);
          if (lk) {
      ...
      // DEBUG: Log extrahierte Spieler
      if (finalHomePlayers.length < 2 || finalAwayPlayers.length < 2) {
...
export async function scrapeMeetingReport({
  ...
  try {
    ...
    // VERBESSERT: Validierung und Logging
    const singlesCount = parsed.singles?.length || 0;
    const doublesCount = parsed.doubles?.length || 0;
    const totalMatches = singlesCount + doublesCount;
    
    console.log(`[nuligaScraper] 📊 Meeting-Report geparst:`, {
      meetingId: meetingId || null,
      url,
      singlesCount,
      doublesCount,
      ...
      metadata: parsed.metadata ? {
        homeTeam: parsed.metadata.homeTeam,
        awayTeam: parsed.metadata.awayTeam
      } : null
    });
    
    // WARNUNG: Wenn keine Matches gefunden wurden
    if (totalMatches === 0) {
      console.warn(`[nuligaScraper] ⚠️ Meeting-Report enthält keine Matches (meetingId: ${meetingId || 'N/A'})`);
    }
    ...
    // 404-Fehler bedeuten, dass das Meeting nicht verfügbar ist (noch nicht gespielt oder gelöscht)
    if (fetchError.status === 404) {
      ...
      error.status = 404;
      error.url = url;
      error.meetingId = meetingId || null;
      throw error;
    }
