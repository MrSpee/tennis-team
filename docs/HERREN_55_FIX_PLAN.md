...
- **Ursache**: Zwei verschiedene `team_info` Einträge mit unterschiedlichen `team_name` Werten (" 1" vs "1")
...
- **Ursache**: 
  ...
  - Filterung in `TeamView.jsx` Zeile 369 filtert nur nach `season`, nicht nach `group_name`
...
- **Ursache**: 
  - `matchdays` Query filtert nach `season` (Zeile 369), aber nicht nach `group_name`
